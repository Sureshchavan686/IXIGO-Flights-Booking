...
public class SearchFlightPOM extends BaseLibrary {
	...
	@FindBy(xpath = "//div[@class='time-group']//div[@class='time'][1]")
	public static List<WebElement> flightDepartureTime;

	@FindBy(xpath = "//div[@class='summary-section']//div[@class='price']//div[@class='c-price-display u-text-ellipsis ']")
	...
	public void getFaredetails() {
		...
		for (int i = 0; i < flightPrices.size(); i++) {
			...
			String fdepartureTime = flightDepartureTime.get(i).getText().trim();
			int Airfare = Integer.parseInt(fare);

			if (Airfare < 7000) {
				System.out.println("Fligt name is: " + flname + " and its fare is: Rs" + Airfare + " Departs at: "
						+ fdepartureTime);
